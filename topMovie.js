module.exports = function () {
    ...
    this.UpdateData = data => {
        ...
        let params = {
            ...
            datas: data,
            model_name: 'topMovie',
            sign: md5(app_key + data + 'topMovie' + 'App.Table.MultiCreate' + app_secrect).toUpperCase()
        };
        console.log('开始导入 TOP排行榜 数据');
            ...
            res.on('end', () => {
                ...
                if (strem.ret == '200') {
                    if(curpage + pagesize < total){
                        console.log(' TOP排行榜 导入成功');
                        curpage += pagesize;
                        obj.GetData();
                    }else{
                        console.log(' TOP排行榜 数据更新完成');
                        uploadLog.UpLog(' TOP排行榜 数据更新完成','INFO');
                    }
                } else {
                    console.log(strem);
                }
    ...
    this.DeleteData = data => {
        ...
        let params = {
            ...
            sign: md5(app_key + 'topMovie' + 'App.Table.FreeDelete' + JSON.stringify([
                ["id", "<>", ""]
    ...
    this.GetData = () => {
        ...
        let options = {
            //hostname:'http://api.douban.com/v2/movie/top250',
        ...
        console.log('开始获取' + curpage + '-' + toCount + ' TOP排行榜 数据');
        let req = http.request(options, function (res) {
            let strem = '';
            res.setEncoding('utf8');
            res.on('data', function (data) {
    ...
    this.GetSqlData=function(start=1,count=20,response){
        let params = {
            ...
            sign: md5(app_key + 'topMovie'+start+count + 'App.Table.FreeQuery' + JSON.stringify([
                ["id", "<>", ""]
            ]) + app_secrect).toUpperCase()
        };
        //console.log(data);
