module.exports = function () {
            ...
            res.on('end', () => {
                ...
                if (typeof strem.subjects != 'undefined') {
                    ...
                    for (let i = 0; i < strem['subjects'].length; i++) {
                        ...
                        if(typeof strem['subjects'][i]['new']!='undefined'){
                            tmp['isNew']=strem['subjects'][i]['new'];
                        }
                        if(typeof strem['subjects'][i]['box']!='undefined'){
                            tmp['box']=strem['subjects'][i]['box'];
                        }
                        if(typeof strem['date']!='undefined'){
                            tmp['collect_date']=strem['date'];
                        }
                        formData.push(tmp);
                    }
    ...
    this.UpdateData = data => {
        ...
        let params = {
            ...
            datas: data,
            model_name: 'usBox',
            sign: md5(app_key + data + 'usBox' + 'App.Table.MultiCreate' + app_secrect).toUpperCase()
        };
        console.log('开始导入 北美票房榜  数据');
        ...
        let req = http.request(options, function (res) {
            ...
            res.on('end', () => {
                strem = JSON.parse(strem);
                if (strem.ret == '200') {
                    console.log(' 北美票房榜  数据更新完成');
                    uploadLog.UpLog(' 北美票房榜 数据更新完成','INFO');
                } else {
                    console.log(strem);
                }
                //myres.write(strem);
    ...
    this.DeleteData = data => {
        ...
        let params = {
            ...
            sign: md5(app_key + 'usBox' + 'App.Table.FreeDelete' + JSON.stringify([
                ["id", "<>", ""]
    ...
    this.GetSqlData=function(start=1,count=20,response){
        let params = {
            ...
            sign: md5(app_key + 'usBox'+start+count + 'App.Table.FreeQuery' + JSON.stringify([
                ["id", "<>", ""]
            ]) + app_secrect).toUpperCase()
        };
        //console.log(data);
